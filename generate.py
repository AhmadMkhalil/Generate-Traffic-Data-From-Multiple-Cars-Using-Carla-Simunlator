...
def reading_data(index, car_id):
    global rgb_info, seg_info
    v_data = []
    ##w_data = []
    k = 0
    w = 0
    rgb_img = cv2.imread(f'custom_data/{car_id}/image'+ str(index)+ '.png', cv2.IMREAD_COLOR)
    seg_img = cv2.imread(f'SegmentationImage/{car_id}/seg'+ str(index)+ '.png', cv2.IMREAD_COLOR)

    if str(rgb_img) != "None" and str(seg_img) != "None":
        # Vehicle
        with open(f'VehicleBBox/{car_id}/bbox'+ str(index), 'r') as fin:
            v_bounding_box_rawdata = fin.read()

        v_bounding_box_data = re.findall(r"-?\d+", v_bounding_box_rawdata)
        v_line_length = int(len(v_bounding_box_data) / 16) 
        ...
        """
        # Walker (Pedestrian)
        with open(f'PedestrianBBox/{car_id}/bbox'+ str(index), 'r') as w_fin:
            w_bounding_box_rawdata = w_fin.read()

        w_bounding_box_data = re.findall(r"-?\d+", w_bounding_box_rawdata)
        w_line_length = int(len(w_bounding_box_data) / 16) 
        ...
        return v_bbox_data, v_line_length, None, None 
...
def processing(img, v_data, w_data, index, car_id):
    global seg_info, area_info
    vehicle_class = 0
    walker_class = 1

    object_area(v_data)
    f = open(f"custom_data/{car_id}/image"+str(index) + ".txt", 'w')
    ...
    for wbbox in w_data:
        ...
        if filtering(w_bb_array, Walker_COLOR) and pre_occluded_objects_excluded(w_bb_array, area_info, Walker_COLOR): 
            ...
            # Ahmad 7 -> 5
            if small_objects_excluded(w_cali_array, 5) and post_occluded_objects_excluded(w_cali_array, Walker_COLOR):
                darknet_x = float((cali_min_x + cali_max_x) // 2) / float(VIEW_WIDTH)
                darknet_y = float((cali_min_y + cali_max_y) // 2) / float(VIEW_HEIGHT)
                darknet_width = float(cali_max_x - cali_min_x) / float(VIEW_WIDTH)
                darknet_height= float(cali_max_y - cali_min_y) / float(VIEW_HEIGHT)
    ...
    """
    f.close()
    cv2.imwrite(f'draw_bounding_box/{car_id}/image'+str(index)+'.png', img)

def run():
    for car_id in range(12):
        dataEA = len(os.walk(f'VehicleBBox/{car_id}/').__next__()[2])
        global rgb_info
        global index_count
        train = open(f"my_data/{car_id}/train.txt", 'w')

        for i in range(dataEA + 1):
            if reading_data(i, car_id) != False:
                v_four_points = converting(reading_data(i, car_id)[0], reading_data(i, car_id)[1])
                ##w_four_points = converting(reading_data(i, car_id)[2], reading_data(i, car_id)[3])
                ##processing(rgb_info, v_four_points, w_four_points, i, car_id)
                processing(rgb_info, v_four_points, None, i, car_id)
                train.write(str(f'custom_data/{car_id}/image'+str(i) + '.png') + "\n")
